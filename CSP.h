...
struct Variable{
	...
	int previous_assignment = 0; /*this variable keeps track of the previous assignment of the variable so it can be 
							   added back into the domain if it is modified*/
	bool initialized = false; /* If true then this cell has been changed from its initial zero state, otherwise it is false*/
};
